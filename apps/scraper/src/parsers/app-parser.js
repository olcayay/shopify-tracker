 ...
 */
export function parseAppPage(html, slug) {
    const $ = cheerio.load(html);
    const jsonLd = parseJsonLd($);
    const appName = jsonLd?.name || $("h1").first().text().trim() || slug;
    const safeParse = (name, fn, fallback) => {
        try {
            return fn();
        }
        catch (e) {
...
function parseAppIntroduction($) {
    ...
    h2s.each((_, el) => {
        ...
        if (text === "Featured images gallery")
            found = true;
    });
    return title;
}
function parseAppDetails($) {
...
function parseSeoTitle($, appName) {
    ...
    if (appName) {
        const prefix = new RegExp(`^${appName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*[-–‑—]\\s*`, "i");
        title = title.replace(prefix, "");
    }
    return title.trim();
}
function parseSeoMetaDescription($) {
...
function parsePricingSummary($) {
    ...
    if (firstCard.length) {
        ...
        if (priceMatch)
            return `From ${priceMatch[0]}/month`;
    }
    return "";
}
function parseDeveloper($) {
    let name = "";
    let url = "";
    let website;
    // Partner link
    $('a[href*="/partners/"]').each((_, el) => {
        const $el = $(el);
        if ($el.closest('.megamenu-component, .side-menu-component').length > 0)
            return;
        if (!name) {
            name = $el.text().trim();
            url = $el.attr("href") || "";
...
function parseCategories($) {
    ...
    $('a[href*="/categories/"]').each((_, el) => {
        const $el = $(el);
        if ($el.closest('.megamenu-component, .side-menu-component, [class*="navbar"]').length > 0)
            return;
 ...
 */
export function parseSimilarApps(html) {
    const $ = cheerio.load(html);
    const result = [];
