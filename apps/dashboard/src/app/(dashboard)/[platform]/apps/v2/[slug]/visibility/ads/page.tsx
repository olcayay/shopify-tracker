...
import { AppAdHistory } from "../../../../[slug]/ads/ad-history";
import { AdHeatmap } from "@/components/ad-heatmap";

export default async function V2AdsPage({
  params,
}: {
          ...
          <CardTitle>
            ...
            {keywordSightings.length > 0 && (
              <span className="text-sm font-normal text-muted-foreground ml-2">
                {new Set(keywordSightings.map((s: any) => s.keywordSlug)).size} keywords, last 30 days
              </span>
            )}
          </CardTitle>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            ...
            {categorySightings.length > 0 && (
              <span className="text-sm font-normal text-muted-foreground ml-2">
                {new Set(categorySightings.map((s: any) => s.categorySlug)).size} categories, last 30 days
              </span>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {categoryHeatmapData.length > 0 ? (
            <AdHeatmap sightings={categoryHeatmapData} linkPrefix={`/${platform}/categories/`} />
          ) : (
