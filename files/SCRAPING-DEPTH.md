...
A `scraper_configs` table (migration `0142_scraper_configs.sql`) stores per-(platform, scraper_type) JSONB overrides on top of the code defaults. At job start the resolver (`apps/scraper/src/config-resolver.ts`) merges code defaults + DB overrides; the result is stored on `scrape_runs.metadata.config_snapshot` for forensics. Empty overrides = "use all defaults" (no behavior change on day 1). The schema registry (`packages/shared/src/scraper-config-schema.ts`) defines which knobs are tunable per scraper type — covers `app_details`, `category`, `keyword_search`, `keyword_suggestions`, `reviews` (Phase 3 PLA-1042 complete). Admin page: `/system-admin/scraper-management` — per-row enable toggle + Configure dialog with dynamic form generated from the schema (Phase 2 PLA-1041). The `enabled` column acts as a per-(platform, type) kill switch, independent of the platform-wide `platform_visibility.scraper_enabled`. Resolver cache TTL is 15s so admin edits propagate to workers within ~15s without explicit pub/sub invalidation (Phase 4 PLA-1043 will add instant invalidation).
