...
describe("BROWSER_PLATFORMS", () => {
  ...
  it("includes salesforce, canva, google_workspace, zoho, zendesk", () => {
    expect(BROWSER_PLATFORMS).toContain("salesforce");
    expect(BROWSER_PLATFORMS).toContain("canva");
    expect(BROWSER_PLATFORMS).toContain("google_workspace");
    expect(BROWSER_PLATFORMS).toContain("zoho");
...
describe("getSmokeCheck", () => {
  it("returns check config for existing platform+check", () => {
    ...
    expect(result!.arg).toBe("finding-products");
  });
